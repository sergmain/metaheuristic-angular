 ...
 */
@Component({
    selector: 'scenario-dtls1',
    templateUrl: 'scenario-dtls1.component.html',
    styleUrls: ['scenario-dtls1.component.css'],
    providers: [ScenarioDetailsService]
})
export class ScenarioDtls1Component implements AfterViewInit {
    @ViewChild('tree') tree;
    ...
    visibleNodes(): DetailNode[] {
        ...
        function addExpandedChildren(node: DetailNode, expanded: string[]) {
            ...
            if (expanded.includes(node.id)) {
                if (ScenarioDtls1Component.isNotNull(node.children)) {
    ...
    private expandNodesById(flatNodes: DetailFlatNode[], ids: string[]) {
        if (ScenarioDtls1Component.isNull(flatNodes) || flatNodes.length === 0) return;
    ...
    static isNotNull(obj: any) {
        return !ScenarioDtls1Component.isNull(obj);
    }
