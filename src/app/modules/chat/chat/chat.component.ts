...
export class ChatComponent extends UIStateComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    hasNoContent = (_: number, _nodeData: StepFlatNode) => {
        ...
        return ChatComponent.anyMode(_nodeData.mode, [NodeMode.new, NodeMode.edit])
    };
    ...
    modeEditNode = (node: StepFlatNode) => {
        let b = ChatComponent.anyMode(node.mode, [NodeMode.edit]);
        //console.log("modeEditNode()", JSON.stringify(node), b);
        return b
    };

    modeNewNode = (node: StepFlatNode) => {
        let b = ChatComponent.anyMode(node.mode, [NodeMode.new]);
...
*/
    ...
    private findNodeWithModeInBranch(datum: SimpleScenarioStep, modes: NodeMode[]) : boolean {
        // console.log("31.10", datum)
        if (ChatComponent.anyMode(datum.mode, modes)) {
            return true;
        }
        if (MhUtils.isNotNull(datum.steps)) {
            for (const child of datum.steps) {
