...
export class AppViewComponent extends UIStateComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    @ViewChild('matSlideToggleTheme', { read: ElementRef }) matSlideToggleTheme: ElementRef | undefined;
    @ViewChild('matSelectLanguage') matSelectLanguage: MatSelect;

    // https://stackoverflow.com/a/75062321/2672202
    ...
    ngAfterViewInit() {
        if (this.matSlideToggleTheme){
            this.matSlideToggleTheme.nativeElement.querySelector('.mdc-switch__icon--on').firstChild.setAttribute('d', this.moon);
            this.matSlideToggleTheme.nativeElement.querySelector('.mdc-switch__icon--off').firstChild.setAttribute('d', this.sun);
        }
    }
